          ...
          if (taskId) {
            this.timer = setInterval(() => {
              if (this.pending === false) {
                ...
                this.$http('fms.commons.getTaskById', { taskId }, { log: false })
                  .then(taskRes => {
                    this.pending = false
                    console.log('taskRes', taskRes)
                    if (taskRes.status === 'finish') {
                      this.progress = 100
                      this.pending = true
                      ...
                      this.fmsDialogClose()
                    } else if (taskRes.status === 'process') {
                      let progress = 0
                      ...
                      clearInterval(this.timer)
                      this.$message.warning(taskRes.exception || taskRes.msg || '操作异常!')
                      this.fmsDialogClose()
                    }
                  })
                  .catch(e => {
